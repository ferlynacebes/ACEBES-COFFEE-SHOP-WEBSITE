...
/* =========================================================
   ...
   Use the exact Active/Inactive values already stored by
   the database so the Edit modal works with either case.
========================================================= */
...
$statusResult = $conn->query("SELECT DISTINCT status FROM products WHERE status IS NOT NULL AND status <> ''");

if ($statusResult) {

    while ($statusRow = $statusResult->fetch_assoc()) {

        $databaseStatus = trim((string) $statusRow["status"]);

        if (strcasecmp($databaseStatus, "Active") === 0) {
            $statusOptions["Active"] = $databaseStatus;
        } elseif (strcasecmp($databaseStatus, "Inactive") === 0) {
            $statusOptions["Inactive"] = $databaseStatus;
        }
    }
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $postedStatus = trim($_POST["status"] ?? "Active");

    /* Convert the form label to the exact database value. */
    $status = $statusOptions[$postedStatus] ?? $postedStatus;
...
$countResult = $conn->query("
    SELECT
        COUNT(*) AS total,
        SUM(LOWER(TRIM(status)) = 'active') AS active,
...
if ($countResult) {
    ...
    $activeProducts = (int) ($counts["active"] ?? 0);
        ...
        <?php if (isset($_GET["error"])): ?>
            ...
            <div class="alert error">
                <?php if (($_GET["error"] ?? "") === "invalid_status"): ?>
                    Invalid product status. Please choose Active or Inactive.
                        ...
                        <?php while ($product = $products->fetch_assoc()): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php if (strcasecmp(trim((string)$product["status"]), "Active") === 0): ?>
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <select name="status" required>
                        ...
                        <?php foreach ($statusOptions as $label => $databaseValue): ?>

                            <option
                                value="<?= htmlspecialchars($label, ENT_QUOTES, "UTF-8") ?>"
                                <?= strcasecmp((string) $selectedStatus, (string) $databaseValue) === 0
                                    || strcasecmp((string) $selectedStatus, (string) $label) === 0
                                    ? "selected"
                                    : "" ?>
                            >
